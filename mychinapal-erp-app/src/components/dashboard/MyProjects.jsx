...
export default function MyProjects({ projects, clientNameById }) {
  ...
  return (
    <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 14, padding: '16px 18px' }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: 'uppercase', letterSpacing: '.4px', marginBottom: 10 }}>Moje aktywne projekty <span style={{ color: C.text2 }}>({projects.length})</span></div>
      {projects.length === 0 && <div style={{ fontSize: 11, color: C.muted }}>Nie masz jeszcze przypisanych projektów — przypisz się w widoku danego zamówienia (sekcja "Zespół").</div>}
      {projects.map(p => (
        <div key={p.id} onClick={() => navigate(`/projekty?project=${p.id}`)}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 10, color: C.muted }}>{clientNameById[p.client_id] || 'Nieznany klient'}</div>
            </div>
          </div>
