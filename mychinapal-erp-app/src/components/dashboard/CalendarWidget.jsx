...
export default function CalendarWidget({ events, profiles, currentUserId, onChanged }) {
  ...
  const handleAdd = async () => {
    ...
    setTitle(''); setDate(''); setAttendeeIds([currentUserId]); setShowAdd(false)
    onChanged && onChanged()
  }
    ...
    <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 14, padding: '16px 18px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span onClick={() => setCursor(c => new Date(c.getFullYear(), c.getMonth() + 1, 1))} style={{ cursor: 'pointer', color: C.muted, fontSize: 12 }}>›</span>
          <span onClick={() => setShowAdd(s => !s)} style={{ fontSize: 11, fontWeight: 700, color: C.blue, cursor: 'pointer', marginLeft: 8 }}>{showAdd ? '✕' : '+ Nowe'}</span>
        </div>
      </div>
      ...
      {showAdd && (
        <div style={{ background: C.bg, borderRadius: 9, padding: 10, marginBottom: 12 }}>
          ...
          <button onClick={handleAdd} disabled={saving} style={{ padding: '7px 14px', borderRadius: 7, border: 'none', background: C.blue, color: '#fff', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>Zapisz spotkanie</button>
        </div>
      )}
        ...
        {cells.map((d, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              fontSize: 10.5, position: 'relative', background: isToday ? C.blue : d ? C.bg : 'transparent',
