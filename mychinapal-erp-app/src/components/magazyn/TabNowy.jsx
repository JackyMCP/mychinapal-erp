...
const DEFAULT_VAT_OPTIONS = ['23%', '8%', '5%', '0%', 'zw.']
...
export default function TabNowy({ products, projects, onChanged, onGoTab, company = 'PL', vatRateOptions }) {
  ...
  const [vatRate, setVatRate] = useState(vatOptions[0])
  ...
  const handleAddProduct = async () => {
    ...
    const { error } = await supabase.from('products').insert({
      ...
      created_by: user?.id, company,
      name_cn: nameCn.trim() || null, name_en: nameEn.trim() || null,
    })
    setSavingProduct(false)
    if (error) { toast.error('Nie udało się dodać towaru: ' + error.message); return }
    setCode(''); setName(''); setNameCn(''); setNameEn(''); setSalePrice(''); setMinStock(''); setIsService(false)
    onChanged && onChanged()
  ...
  return (
    <div>
      <div style={card}>
        <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: 14 }}>{t("Nowy towar w kartotece")} {isCN ? '🇨🇳' : ''}</div>
        <div style={fieldWrap}>
          ...
          {isCN && (
            <>
              <div><label style={label}>{t("Nazwa chińska (中文品名)")}</label><input style={input} value={nameCn} onChange={e => setNameCn(e.target.value)} placeholder="例如 玻璃杯" /></div>
              <div><label style={label}>{t("Nazwa angielska (na fakturze eksportowej)")}</label><input style={input} value={nameEn} onChange={e => setNameEn(e.target.value)} placeholder="e.g. Glass tumbler" /></div>
            </>
          )}
          <div><label style={label}>{t("Jednostka miary")}</label>
            <select style={input} value={unit} onChange={e => setUnit(e.target.value)}>
              ...
              {isCN && <option value="个/PCS">个/PCS</option>}
            </select>
          </div>
          ...
          <div><label style={label}>{t("Cena sprzedaży netto")} {isCN && '(CNY)'}</label><input style={input} value={salePrice} onChange={e => setSalePrice(e.target.value)} placeholder="0,00" /></div>
      ...
      <div style={card}>
        ...
        <div style={fieldWrap}>
          ...
          <div><label style={label}>{t("Cena zakupu (netto, jednostkowa)")} {isCN && '(CNY)'}</label><input style={input} value={pzPrice} onChange={e => setPzPrice(e.target.value)} placeholder="0,00" /></div>
